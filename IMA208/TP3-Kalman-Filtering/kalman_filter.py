...
class KalmanFilter(object):
    ...
    def __init__(self, dim_x, dim_z):
        ...
        self.Q = np.identity( dim_x )       # process uncertainty
        self.F = np.identity( dim_x )       # state transition matrix
        self.H = np.zeros( (dim_z, dim_x) ) # measurement matrix
        self.R = np.identity( dim_z )       # measurement uncertainty
    ...
    def update(self, z, R=None, H=None):
        ...
        self.P = self.P - self.P@(H.T)@np.linalg.inv(S)@H@self.P