...
class ParticleFilter(object):
    ...
    def resample(self, tau=None):
        ...
        if N_eff <= threshold:
            ...
            idx = np.random.choice( np.arange( self.n_particles ), size=self.n_particles, replace=True, p=self.weights )
    ...
    def state_expectation(self):
        ...
        """
        mean = np.sum( self.particles * self.weights.reshape( self.n_particles, 1 ), axis=0 )
